...
if "engine" not in st.session_state:
    # 检查是否有上次保存的进度
    if has_saved_progress() and "resume_choice" not in st.session_state:
        saved = load_progress()
    ...
    # 恢复上次进度
    if st.session_state.get("resume_choice") == "continue":
        saved = load_progress()
        if saved:
